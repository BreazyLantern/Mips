...
loop:		lb $t2, 0($t0) #loads the first byte of the word into $t2
		...
		beq $t2, 10, done #if the current byte is a null space, go to exit
		
		addi $t1, $t1,1
		j loop 
		
done:		move $v1, $t1 #moves the result of counter to $v0 so the main part of the program can "see" it
		jr	$ra
